...
class recruiterFetch:
    ...
    def scrape(
        ...
        previous_start_param = "start=0"
        previous_lead_count = 0
        lead_change_counter = 0
        ...
        for num_candidates in range(can_start, can_end, can_count):
            ...
            url = url.replace(previous_start_param, f"start={num_candidates}")
            ...
            if lead_change_counter < 3:
                if previous_lead_count == len(self.m_leads):
                    lead_change_counter += 1
                else:
                    lead_change_counter = 0
            else:
            ...
            previous_start_param = f"start={num_candidates}"
            previous_lead_count = len(self.m_leads)
            print(f"Scraped {previous_lead_count} leads")
